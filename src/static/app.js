        ...
        const activityCard = document.createElement("div");
          ...
          const participantsHtml = details.participants && details.participants.length > 0
            ? `<h5>Participants (${details.participants.length})</h5>` +
              `<ul class="participants-list">${details.participants.map(p => `<li><span class="participant-email">${p}</span> <button class="remove-participant" data-email="${p}" aria-label="Remove ${p}">✖</button></li>`).join('')}</ul>`
